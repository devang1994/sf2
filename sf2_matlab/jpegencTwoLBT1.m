function [vlc bits huffval] = jpegencTwoLBT1(X, qstep, N, M,opthuff, dcbits)
    
%trying out block by block encoding and everything on 2 layer lbt useless
...
for r=0:M:(sy(1)-M),%for rows
  ...
  for c=0:M:(sy(2)-M),%for columns
    ...
    yq=twoLayerLBTenc(X,4,1.3);
    
    yq=quant1(yq,qstep,qstep);
...
kB=(sum(vlc(:,2))/(1024*8));
fprintf(1,'kBits for coded image = %d kB\n', kB)
fprintf(1,'Bits for huffman table = %d bits\n', (16+max(size(dhuffval)))*8)
disp('I am edited');
if (nargout>1)
  bits = dbits;
  huffval = dhuffval';
